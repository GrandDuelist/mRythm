...
class TravelTimeSpark(Spark):
    ...
    def tapInTapOutVsDensityRatioMultiMonthInStation(self,density_input_dir,delay_input_dir,local_output_dir,file_prefix):
        client = Config().get_client('dev',)
        days = client.list(density_input_dir)
        m = {}
        results = {}
        for day in days:
            try:
                ...
                density_file_path = density_input_dir + day
                delay_file_path = delay_input_dir+day
                delay_ratio = self.tapInTapOutVsDensityRatioInStation(density_file_path,delay_file_path,month)
                # delay_ratio.saveAsTextFile('/zf72/spark-results/subway/delay-ratio-one-user/%s' % month)
                m[month] = True
                print('INFO: Processing time %s' % day)
            except Exception as e:
    ...
    def tapInTapOutVsDensityRatioInStation(self,file_path_density=None,file_path_delay_ratio=None,month=""):
        self.spark_interval.sc = self.sc
        self.tapin_tapout_data = self.sc.textFile(file_path_density,use_unicode=False).cache()
        self.delay_ratio = self.sc.textFile(file_path_delay_ratio,use_unicode=False).cache()
        density = self.tapin_tapout_data.map(self.spark_interval.subway.parseTuple).map(lambda x: ((x[0],x[1]),x))
        delay = self.tapin_tapout_data.map(self.spark_interval.subway.parseTuple).map(lambda x: ((x[0],x[2]),x))
        combined = delay.join(density).map(lambda (k,v): (v[0][0],v[0][1],v[0][2],v[0][3],v[1][2]))
        combined = combined.map(lambda x: ','.join([str(v) for v in x]))
        combined.saveAsTextFile('/zf72/spark-results/subway/density-delay-ratio-in-station/%s' % month)
        return(combined)

    def passengerTapOutInStations(self,file_path=None,month=""):
        if file_path is None:
            # file_path = self.public_sample
            file_path = self.subway_one_day
        self.setHDFSFilePath(file_path)
        ...
        self.spark_interval.buildTripList()
        self.spark_interval.buildNoUserTripList()
        passenger_density = self.spark_interval.tapOutPerStationPerTimeUnit()
        passenger_density = passenger_density.map(lambda x: ','.join([str(x[0]),str(x[1]),str(x[2])]))
        passenger_density.saveAsTextFile('/zf72/spark-results/subway/passenger-station-tap-out/%s' % month)
        return(passenger_density)


    def passengerTapOutInStationsMultiMonths(self,input_dir,local_output_dir,file_prefix):
        client = Config().get_client('dev',)
        days = client.list(input_dir)
        m = {}
        results = {}
        for day in days:
            try:
                ...
                if month in m:
                    ...
                    continue
                file_path = input_dir + day
                delay_ratio = self.passengerTapOutInStations(file_path,month)
                # delay_ratio.saveAsTextFile('/zf72/spark-results/subway/delay-ratio-one-user/%s' % month)
                count = delay_ratio.map(lambda x: x[2]).count()
                total = delay_ratio.map(lambda x: x[2]).sum()
                average = float(total)/float(count)
                ...
                print("ERROR: file %s is damaged" % file_path)
                print(e)
        json.dump(results,open(local_output_dir+"monthly-tap-out-region.json",'w'))
